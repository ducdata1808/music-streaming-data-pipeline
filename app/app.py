...
menu = st.sidebar.selectbox(
    ...
    ["Overview", "Top Artists", "Music listening trends by time", "Top listening trends by areas", "For You"]
)
...
# For You (Recommendations)
    ...
    st.write("Select a User ID to see top 10 recommended songs generated by the ALS model.")
    
    try:
        # Check if table exists and get unique users
        users_df = client.query_df("SELECT DISTINCT user_id FROM recommendations ORDER BY user_id")
        user_list = users_df['user_id'].tolist() if not users_df.empty else []
        
        if not user_list:
            st.warning("No recommendations available yet. Run the ALS model on Airflow first to generate data.")
            st.stop()
        ...
        if user_id:
            query = f"""
                SELECT rec_rank as Rank, song as Song, artist as Artist, round(score, 2) as Score
                FROM recommendations
                WHERE user_id = '{user_id}'
                ORDER BY rec_rank ASC
            """
            df_recs = client.query_df(query)
            
            st.write(f"### Top 10 songs for User `{user_id}`")
            st.dataframe(df_recs, use_container_width=True)
            
            verbose_output(f"{Background_colors.GREEN}Successfully retrieved recommendations for user {user_id}{Background_colors.END}")
            
    except Exception as e:
        st.error(f"Error fetching recommendations. It seems the `recommendations` table does not exist in ClickHouse yet.")
        st.info("💡 Lời nhắc: Bạn cần tạo bảng recommendations trong ClickHouse trước! Mở terminal và chạy lệnh sau:\n\n`clickhouse-client --queries-file ~/eventsim_project/scripts/clickhouse_init_local.sql`")
