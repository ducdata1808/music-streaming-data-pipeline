...
# For You (Recommendations)
    ...
    st.write("Enter a User ID to see top 10 recommended songs generated by the ALS model.")
    
    user_id = st.text_input("User ID:", placeholder="e.g. 10")
    ...
    if user_id:
        try:
            ...
            if df_recs.empty:
                st.warning(f"No recommendations found for user `{user_id}`. Run the ALS model on Airflow first to generate data.")
            else:
                st.write(f"### Top 10 songs for `{user_id}`")
            ...
            st.error(f"Error fetching recommendations (Ensure the recommendations table exists and Airflow job has run): {e}")
